// Love Story Website - Enhanced with Performance & Security
(function() {
  'use strict'
  ...
  document.addEventListener('DOMContentLoaded', () => {
    if (typeof process === 'undefined' || process.env.NODE_ENV !== 'production') {
      const domContentTime = performance.now() - startTime
      // eslint-disable-next-line no-console
      console.log(`DOMContentLoaded: ${domContentTime.toFixed(2)}ms`)
    }
    ...
    // Preloader premium with error handling
    ...
    // Enhanced cursor with error handling
    const cursor = document.querySelector('.cursor')
    const cursorFollower = document.querySelector('.cursor-follower')
    let mouseX = 0, mouseY = 0
    let cursorX = 0, cursorY = 0

    // Smooth cursor following function
    ...
    // Enhanced hover effects with error handling
      ...
      if (calculateBtn) {
        calculateBtn.addEventListener('click', () => {
          ...
          if (name1.length < 2 || name2.length < 2) {
            alert('Os nomes devem ter pelo menos 2 caracteres! 💕')
            return
          }

          if (name1.length > 50 || name2.length > 50) {
            alert('Os nomes são muito longos! Máximo 50 caracteres. 💕')
            return
          }
          ...
          if (!namePattern.test(name1) || !namePattern.test(name2)) {
            alert('Os nomes devem conter apenas letras, espaços e hífens! 💕')
            return
          }
